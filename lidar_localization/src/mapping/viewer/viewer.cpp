...
bool Viewer::InitFilter(std::string filter_user, std::shared_ptr<CloudFilterInterface>& filter_ptr,
...
bool Viewer::SaveMap()
{
  if (optimized_key_frames_.size() == 0){
    std::cout << "optimized_key_frames_.size():" << optimized_key_frames_.size() << std::endl;
    return false;
  }
  // 生成地图
