...
ViewerFlow::ViewerFlow(ros::NodeHandle& nh, std::string cloud_topic)
{
  ...
  transformed_fused_odom_sub_ptr_     = std::make_shared<OdometrySubscriber>(nh, "/transformed_fused_odom", 100000);
...
bool ViewerFlow::Run()
{
  ...
  while (HasData())
  {
    if (ValidData())
    {
      viewer_ptr_->UpdateWithNewKeyFrame(key_frame_buff_, current_transformed_fused_odom_, current_cloud_data_);
      PublishLocalData();
    }
  }
...
bool ViewerFlow::ReadData()
{
  ...
  transformed_fused_odom_sub_ptr_->ParseData(transformed_fused_odom_buff_);
...
bool ViewerFlow::HasData()
{
  if (cloud_data_buff_.size() == 0)
    return false;
  if (transformed_fused_odom_buff_.size() == 0)
    return false;
...
bool ViewerFlow::ValidData()
{
  ...
  current_transformed_fused_odom_ = transformed_fused_odom_buff_.front();

  double diff_fused_odom_time = current_cloud_data_.time - current_transformed_fused_odom_.time;

  // std::cout << "××××××××diff_fused_odom_time××××××:"<< diff_fused_odom_time << std::endl;

  if (diff_fused_odom_time < -0.05)
  {
    cloud_data_buff_.pop_front();
    return false;
  }
  if(diff_fused_odom_time > 0.05){
    transformed_fused_odom_buff_.pop_front();
    return false;
  }
