...
bool BackEnd::InitDataPath(const YAML::Node &config_node)
{
  ...
  if (!FileManager::CreateFile(laser_odom_ofs_, trajectory_path_ + "/laser_odom.txt"))
    return false;
  if (!FileManager::CreateFile(fused_odom_ofs_, trajectory_path_ + "/fused_odom.txt"))
    return false;

  return true;
}

bool BackEnd::Update(const CloudData &cloud_data, const PoseData &laser_odom, const PoseData &fused_odom, const PoseData &gnss_pose)
{
  ResetParam();

  if (MaybeNewKeyFrame(cloud_data, fused_odom, gnss_pose))  //  使用ESKF的结果作为图优化的输入
  {
    SavePose(ground_truth_ofs_, gnss_pose.pose);
    SavePose(laser_odom_ofs_, laser_odom.pose);
    SavePose(fused_odom_ofs_, fused_odom.pose);
