...
{
BackEndFlow::BackEndFlow(ros::NodeHandle &nh, std::string cloud_topic, std::string laser_odom_topic, std::string fused_odom_topic)
{
  ...
  laser_odom_sub_ptr_        = std::make_shared<OdometrySubscriber>(nh, laser_odom_topic, 100000);
  fused_odom_sub_ptr_        = std::make_shared<OdometrySubscriber>(nh, fused_odom_topic, 100000);
  loop_pose_sub_ptr_         = std::make_shared<LoopPoseSubscriber>(nh, "/loop_pose", 100000);

  // transformed_laser_odom_pub_ptr_  = std::make_shared<OdometryPublisher>(nh, "/transformed_laser_odom", "/map", "/lidar", 100); //lidar里程计挂到gnss初始系下的位姿
  transformed_fused_odom_pub_ptr_  = std::make_shared<OdometryPublisher>(nh, "/transformed_fused_odom", "/map", "/lidar", 100);
...
bool BackEndFlow::HasData()
{
  ...
  if (laser_odom_data_buff_.size() == 0)
    return false;
  if (fused_odom_data_buff_.size() == 0)
    return false;
...
bool BackEndFlow::ValidData()
{
  ...
  double diff_fused_time = current_cloud_data_.time - current_fused_odom_data_.time;

  if (diff_gnss_time < -0.05 || diff_laser_time < -0.05 || diff_fused_time < -0.05)
  {
  ...
  if (diff_fused_time > 0.05)
  {
    fused_odom_data_buff_.pop_front();
    return false;
  }
...
bool BackEndFlow::UpdateBackEnd()
{
  ...
  // static Eigen::Matrix4f fused_init_pose = Eigen::Matrix4f::Identity();

  // if (!odometry_inited)
  // {
  //   odometry_inited = true;
  //   odom_init_pose  = current_gnss_pose_data_.pose * current_laser_odom_data_.pose.inverse();
  //   fused_init_pose = current_gnss_pose_data_.pose * current_fused_odom_data_.pose.inverse();
  // }
  // current_laser_odom_data_.pose = odom_init_pose * current_laser_odom_data_.pose;
  // current_fused_odom_data_.pose = fused_init_pose * current_fused_odom_data_.pose;

  std::cout<< "++++++++++++++++++++++++++++后端优化之前"<< std::endl;
  return back_end_ptr_->Update(current_cloud_data_, current_laser_odom_data_, current_fused_odom_data_, current_gnss_pose_data_);
}

bool BackEndFlow::PublishData()
{
  // transformed_laser_odom_pub_ptr_->Publish(current_laser_odom_data_.pose, current_fused_odom_data_.time); //两者时间是否一致 current_laser_odom_data_.time
  transformed_fused_odom_pub_ptr_->Publish(current_fused_odom_data_.pose, current_fused_odom_data_.time); //current_fused_odom_data_.time
  double diff_time = current_laser_odom_data_.time - current_fused_odom_data_.time;
  std::cout<< "++++++++++++++++++++++++++++laser-fused之间的时间差："<< diff_time << std::endl;
