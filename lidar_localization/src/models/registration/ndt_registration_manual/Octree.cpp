#include "lidar_localization/models/registration/ndt_registration_manual/Octree.h"
...
//template <typename CloudData::POINT>
Octree::Octree()
{
	leaf_x_ = 4; //16;
	leaf_y_ = 4; //16;
...
//template <typename CloudData::POINT>
int Octree::roundUp(int input, int factor)
{
	return (input < 0) ? (-((-input) / factor) * factor) : ((input  + factor - 1 ) / factor) * factor;
}

//template <typename CloudData::POINT>
int Octree::roundDown(int input, int factor)
{
	return (input < 0) ? (-(-input + factor - 1) / factor) * factor : (input / factor) * factor;
}

//template <typename CloudData::POINT>
int Octree::div(int input, int divisor)
{
	return (input < 0) ? (-(-input + divisor - 1) / divisor) : (input / divisor);
}

//template <typename CloudData::POINT>
void Octree::setInput(std::vector<Eigen::Vector3i> occupied_voxels, const CloudData::CLOUD_PTR point_cloud)
{
	octree_.reset();
	reserved_size_.reset();
	dimension_.reset();
	...
	for (unsigned int i = 0; i < occupied_voxels.size(); i++) {
	...
	for (unsigned int i = 0; i < occupied_voxels.size(); i++) {
		...
		CloudData::POINT p = point_cloud->points[i];
	...
	for (unsigned int level = 1; level < (*octree_).size(); level++) {
		buildLevel(level);
	}
}

//template <typename CloudData::POINT>
bool Octree::isOccupied(int node_id, int level)
{
...
//template <typename CloudData::POINT>
bool Octree::isOccupied(std::vector<unsigned int> occupancy, int node_id)
{
	int val_loc = node_id / (sizeof(int) * 8);
	int bit_loc = node_id % (sizeof(int) * 8);
...
//template <typename CloudData::POINT>
void Octree::setOccupied(int node_id, int level)
{
	std::vector<unsigned int> &current_occ = (*occupancy_check_)[level];
	int val_loc = node_id / (sizeof(int) * 8);
	int bit_loc = node_id % (sizeof(int) * 8);
...
//template <typename CloudData::POINT>
void Octree::setOccupied(std::vector<unsigned int> &occupancy, int node_id)
{
	int val_loc = node_id / (sizeof(int) * 8);
	int bit_loc = node_id % (sizeof(int) * 8);
...
//template <typename CloudData::POINT>
void Octree::buildLevel(int level)
{
	// Parent list
...
//template <typename CloudData::POINT>
void Octree::update(std::vector<Eigen::Vector3i> new_voxels, const CloudData::CLOUD_PTR new_cloud)
{
...
//template <typename CloudData::POINT>
void Octree::updateBoundaries(std::vector<Eigen::Vector3i> new_voxels)
{
	...
	for (unsigned int i = 0; i < new_voxels.size(); i++) {
	...
	if (dst_bounds.lower_x < src_bounds.lower_x || dst_bounds.lower_y < src_bounds.lower_y || dst_bounds.lower_z < src_bounds.lower_z ||
		...
		for (; level < (*old_octree).size(); level++) {
			...
			for (unsigned int i = 0; i < src_occupancy.size(); i++) {
...
//template <typename CloudData::POINT>
int Octree::index2id(int idx, int idy, int idz, int level)
{
...
//template <typename CloudData::POINT>
int Octree::index2id(int idx, int idy, int idz, OctreeLevelBoundaries bounds, OctreeLevelDim dim)
{
	return ((idx - bounds.lower_x) + (idy - bounds.lower_y) * dim.x + (idz - bounds.lower_z) * dim.x * dim.y);
}

//template <typename CloudData::POINT>
Eigen::Vector3i Octree::id2index(int id, int level)
{
...
//template <typename CloudData::POINT>
Eigen::Vector3i Octree::id2index(int id, OctreeLevelBoundaries bounds, OctreeLevelDim dims)
{
	Eigen::Vector3i output;
...
//template <typename CloudData::POINT>
void Octree::updateOctreeContent(std::vector<Eigen::Vector3i> new_voxels, const CloudData::CLOUD_PTR new_cloud)
{
	for (unsigned int i = 0; i < new_voxels.size(); i++) {
		Eigen::Vector3i vid = new_voxels[i];
		...
		for (unsigned int level = 0; level < (*octree_).size(); level++) {
...
//template <typename CloudData::POINT>
Eigen::Matrix<float, 6, 1> Octree::nearestOctreeNode(CloudData::POINT q)
{
...
//template <typename CloudData::POINT>
double Octree::dist(OctreeNode node, CloudData::POINT p)
{
	double dx, dy, dz;
...
//template <typename CloudData::POINT>
void Octree::initRange(CloudData::POINT q, double &min_range, int &nn_id)
{
...
// Check all childrens
//template <typename CloudData::POINT>
void Octree::goDown(Eigen::Matrix<int, 4, 1> node, CloudData::POINT q, double &min_range, int &current_nn_voxel)
{
...
//template <typename CloudData::POINT>
void Octree::goUp(Eigen::Matrix<int, 4, 1> node, CloudData::POINT q, double &min_range, int &current_nn_voxel)
{
