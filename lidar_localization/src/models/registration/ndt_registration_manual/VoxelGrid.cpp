#include "lidar_localization/models/registration/ndt_registration_manual/VoxelGrid.h"
...
#include "lidar_localization/models/registration/ndt_registration_manual/SymmetricEigenSolver.h"
...
void VoxelGrid::initialize()
{
	...
	staticvalue_.reset();
	staticvalue_ = boost::make_shared<std::vector<double> >(voxel_num_);
	...
	tmp_staticvalue_.reset();
	tmp_staticvalue_ = boost::make_shared<std::vector<double> >(voxel_num_);
}
...
double VoxelGrid::getSaticValue(int voxel_id) const{
	return (*staticvalue_)[voxel_id];
}
...
int VoxelGrid::voxelId(CloudData::POINT p)
{
...
void VoxelGrid::scatterPointsToVoxelGrid()
{
	...
	for (unsigned int pid = 0; pid < source_cloud_->points.size(); pid++) {
		int vid = voxelId(source_cloud_->points[pid]);
		CloudData::POINT p = source_cloud_->points[pid];
		...
		if ((*points_id_)[vid].size() == 0) {
			...
			(*staticvalue_)[vid] = 0.0;     //加的
			(*tmp_staticvalue_)[vid] = 0.0; //加的
		}
		...
		(*tmp_staticvalue_)[vid] += p.intensity;//加的
	}
}

//计算每个cell内点的均值、协方差矩阵、静态权重
	...
	for (int idx = real_min_bx_; idx <= real_max_bx_; idx++)
		for (int idy = real_min_by_; idy <= real_max_by_; idy++)
			for (int idz = real_min_bz_; idz <= real_max_bz_; idz++) {
				...
				double st_sum          = (*tmp_staticvalue_)[i];

				if (ipoint_num > 0) {
					(*centroid_)[i]    = pt_sum / point_num;
					(*staticvalue_)[i] = st_sum / point_num;
				}
...
void VoxelGrid::setInput(CloudData::CLOUD_PTR input_cloud)
{
	if (input_cloud->points.size() > 0) {
		...
		findBoundaries(); //得到 voxel 总数

		std::vector<Eigen::Vector3i> voxel_ids(input_cloud->points.size());//2、确定 VoxelGrid 每个维度的 size

		// 获取每个点的 voxel id，[x,y,z] 三个维度上的 id 确定一个 voxel
		for (unsigned int i = 0; i < input_cloud->points.size(); i++) {
			Eigen::Vector3i &vid = voxel_ids[i];
			// CloudData::POINT p = input_cloud->points[i];
			const CloudData::POINT &p = input_cloud->points[i];
		...
		octree_.setInput(voxel_ids, input_cloud);//3、调用 Octree 的 setInput 进行初始化

		voxel_ids.clear();
		

		initialize();
		scatterPointsToVoxelGrid(); //计算中间值
		computeCentroidAndCovariance();//计算每个 voxel 的 centroid（μ）, covariance（Σ）以及cell的静态权重
	}
}
...
void VoxelGrid::findBoundaries(const CloudData::CLOUD_PTR input_cloud,
...
void VoxelGrid::radiusSearch(CloudData::POINT p, float radius, std::vector<int> &voxel_ids, int max_nn)
{
	float t_x = p.x;
	float t_y = p.y;
	float t_z = p.z;
...
// max_range 没有用，不知道为啥
int VoxelGrid::nearestVoxel(CloudData::POINT query_point, Eigen::Matrix<float, 6, 1> boundaries, float max_range)
{
...
double VoxelGrid::nearestNeighborDistance(CloudData::POINT q, float max_range)
{
...
void VoxelGrid::update(const CloudData::CLOUD_PTR new_cloud)
{
	...
	for (unsigned int i = 0; i < new_cloud->points.size(); i++) {
		Eigen::Vector3i &vid = new_voxel_id[i];
		CloudData::POINT p = new_cloud->points[i];
													...
													float min_x, float min_y, float min_z)
{
	...
	if (new_max_x > max_x_ || new_max_y > max_y_ || new_max_z > max_z_ ||
		...
		if (max_b_x > max_b_x_ || max_b_y > max_b_y_ || max_b_z > max_b_z_ ||
			...
			boost::shared_ptr<std::vector<double> > old_staticvalue     = staticvalue_;
			boost::shared_ptr<std::vector<double> > old_tmp_staticvalue = tmp_staticvalue_;
			...
			staticvalue_     = boost::make_shared<std::vector<double> >(voxel_num);//
			tmp_staticvalue_ = boost::make_shared<std::vector<double> >(voxel_num);//加的
				...
				for (int idy = real_min_by_; idy <= real_max_by_; idy++) {
					for (int idz = real_min_bz_; idz <= real_max_bz_; idz++) {
						...
						if ((*old_points_id)[old_id].size() > 0) {
							...
							(*staticvalue_)[new_id] = (*old_staticvalue)[old_id];//

							(*tmp_centroid_)[new_id]    =  (*old_tmp_centroid)[old_id];
							(*tmp_staticvalue_)[new_id] =  (*old_tmp_staticvalue)[old_id];
...
void VoxelGrid::updateVoxelContent(const CloudData::CLOUD_PTR new_cloud)
{
	int total_points_num = source_cloud_->points.size();

	for (unsigned int i = 0; i < new_cloud->points.size(); i++) {
		// CloudData::POINT p = new_cloud->points[i];
		const CloudData::POINT &p = new_cloud->points[i];
		Eigen::Vector3d p3d(p.x, p.y, p.z);
		...
		if (tmp_pid.size() == 0) {
			(*centroid_)[vid].setZero();
			(*staticvalue_)[vid] = 0.0;  //
			(*tmp_cov_)[vid].setIdentity();
			(*tmp_centroid_)[vid].setZero();
			(*tmp_staticvalue_)[vid] = 0.0; //
		}
		...
		(*staticvalue_)[vid] = (*tmp_staticvalue_)[vid] / static_cast<double>(ipoint_num);
		...
		(*staticvalue_)[vid] = (*tmp_staticvalue_)[vid] / point_num;
