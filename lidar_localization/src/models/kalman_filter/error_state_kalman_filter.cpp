...
ErrorStateKalmanFilter::ErrorStateKalmanFilter(const YAML::Node &node){
    ...
    COV.PROCESS.BIAS_FLAG  = node["covariance"]["process"]["bias_flag"].as<bool>();
...
bool ErrorStateKalmanFilter::GetAngularDelta(const size_t index_curr,
  ...
  angular_delta = 0.5 * delta_t * (angular_vel_curr + angular_vel_prev);  // k-1到k时刻的 相对旋转 对应的 等效旋转矢量

  angular_vel_mid = 0.5 * (angular_vel_curr + angular_vel_prev);  // 角速度均值 b系下
  return true;
}
...
void ErrorStateKalmanFilter::UpdateOrientation(
  ...
  // write back:
  R_prev = pose_.block<3, 3>(0, 0); //body2world
  pose_.block<3, 3>(0, 0) = q.normalized().toRotationMatrix();
  R_curr = pose_.block<3, 3>(0, 0); //body2world
}
...
bool ErrorStateKalmanFilter::GetVelocityDelta(
  ...
  linear_acc_prev = GetUnbiasedLinearAcc(linear_acc_prev, R_prev);             //   转换到w系下

  // mid-value acc can improve error state prediction accuracy:
  linear_acc_mid = 0.5 * (linear_acc_curr + linear_acc_prev);  //世界系下的呀
...
void ErrorStateKalmanFilter::SetProcessEquation(const Eigen::Matrix3d &C_nb,                      //   更新状态方程  F矩阵
  ...
  F_.setZero();
  F_.block<3,  3>(kIndexErrorPos,  kIndexErrorVel)  =  Eigen::Matrix3d::Identity();
  F_.block<3,  3>(kIndexErrorVel,   kIndexErrorOri)   =  -C_nb *  Sophus::SO3d::hat(f_b).matrix();  // f_b是b系下的
...
void ErrorStateKalmanFilter::UpdateErrorEstimation(                       //  更新误差值
  ...
  UpdateProcessEquation(linear_acc_mid ,  angular_vel_mid);  //linear_acc_mid应该是在b系下
 ...
 * @return void
 */
void ErrorStateKalmanFilter::CorrectErrorEstimationPose(                    //  计算  Y, G, C, K
    const Eigen::Matrix4d &T_nb, Eigen::VectorXd &Y, Eigen::MatrixXd &G,
    Eigen::MatrixXd &K) {
  //
  // TODO: set measurement:      计算观测 delta pos  、 delta  ori
  //
  Eigen::Vector3d  dp  =  pose_.block<3,  1>(0,  3)  -   T_nb.block<3,  1>(0,  3); //预测值-观测值
  Eigen::Matrix3d  dR  =  T_nb.block<3,  3>(0, 0).transpose() *  pose_.block<3, 3>(0, 0) ; //观测值的转置*预测值
